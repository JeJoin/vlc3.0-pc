...
const static uint8_t hanzi_dict[256] =
{
    ,
};

void libvlc_media_set_secret( libvlc_media_t *p_md, const char *psz_secret)
{
    assert( p_md );
    int i, n;
    const char * secret = psz_secret;
    char * psecret = NULL;
    
    if (secret != NULL && strlen(secret) > 0) {
        n = strlen(secret);
        psecret = strdup(secret);
        
        for (i = 0; i < n; i++) {
            psecret[i] = (char)hanzi_dict[(uint8_t)secret[i]];
        }
        
        input_item_SetEncryptSecret(p_md->p_input_item, psecret);
        free(psecret);
    } else {
        input_item_SetEncryptSecret(p_md->p_input_item, NULL);
    }
}

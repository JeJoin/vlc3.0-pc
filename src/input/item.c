...
void input_item_SetEncryptSecret( input_item_t *p_i, const char * psz_secret )
{
    vlc_mutex_lock( &p_i->lock );
    if (p_i->psz_secret != NULL) {
        free(p_i->psz_secret);
        p_i->psz_secret = NULL;
    }

    if (psz_secret != NULL) {
        p_i->psz_secret = strdup(psz_secret);
    }
    vlc_mutex_unlock( &p_i->lock );
}

char * input_item_GetEncryptSecret( input_item_t *p_i )
{
    char * secret = NULL;

    vlc_mutex_lock( &p_i->lock );
    if (p_i->psz_secret != NULL) {
        secret = strdup(p_i->psz_secret);
    }
    vlc_mutex_unlock( &p_i->lock );
...
void input_item_Release( input_item_t *p_item )
{
    ...
    if (p_item->psz_secret != NULL) {
        free (p_item->psz_secret);
        p_item->psz_secret = NULL;
    }

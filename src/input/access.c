                            ...
                            bool preparsing, const char *mrl)
{
    ...
    char * psz_secret = input_item_GetEncryptSecret( priv->p_item );
    if (psz_secret != NULL) {
        access->i_secret = strlen(psz_secret);
        access->psz_secret = psz_secret;
    }
                           ...
                           bool preparsing, const char *url)
{
    ...
    s->p_sys      = access;
    if (access->i_secret > 0 && access->psz_secret != NULL) {
        s->i_secret = access->i_secret;
        s->psz_secret = strdup(access->psz_secret);
    }

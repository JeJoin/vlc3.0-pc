...
//    }
    ...
    block_t * p_newblock = p_block;
    if(p_sys->pf_process_block)
    {
        p_newblock = p_sys->pf_process_block(p_dec, p_block, &b_config_changed);
        if (!p_newblock)
            return VLCDEC_SUCCESS;
    }

    frame_info_t *p_info = CreateReorderInfo(p_dec, p_newblock);
    if(unlikely(!p_info))
        goto skip;
    ...
    CMSampleBufferRef sampleBuffer =
        VTSampleBufferCreate(p_dec, p_sys->videoFormatDescription, p_newblock);
    ...
    if (p_sys->b_enable_temporal_processing
     && (p_newblock->i_flags & BLOCK_FLAG_INTERLACED_MASK))
    ...
    if (HandleVTStatus(p_dec, status, &vtsession_status) == VLC_SUCCESS)
    {
        ...
        if( p_newblock->i_flags & BLOCK_FLAG_END_OF_SEQUENCE )
            Drain( p_dec, false );
    }
    else
    {
         ...
         || vtsession_status == VTSESSION_STATUS_RESTART_DECAGAIN)
        {
             ...
             && vtsession_status == VTSESSION_STATUS_RESTART_DECAGAIN)
            {
                 ...
                 * Decoder Callback */
                p_info = CreateReorderInfo(p_dec, p_newblock);
                if (likely(p_info))
...
reload:
    if (p_newblock != NULL && p_newblock != p_block) {
        block_Release(p_newblock);
    }
    if (p_info != NULL) {
        free(p_info);

...
/* */
static int Connect( stream_t * , const char * extheader);
...
static void printHex(const char * buf, int n)
{
    ...
    for (i = 0; i < m; i++) {
        off += sprintf(out + off, "%02x ", (unsigned char)buf[i]);
    }
    sdk_log(6, "hanzi", out);
}
...
static ssize_t Read( stream_t *p_access, void *p_buffer, size_t i_len )
{
    access_sys_t *p_sys = p_access->p_sys;
    int i_read;
    ssize_t nkey = p_access->i_secret;
    const char * psz_key = p_access->psz_secret;
    ...
    assert( i_read >= 0 );
    //printf("read data:%d\n", i_read);
    //printHex(p_buffer, i_read);
    if (nkey > 0 && psz_key != NULL) {
        char * buffer = (char *)p_buffer;
        off_t curoff = p_sys->offset % nkey;
        
        for (ssize_t i = 0; i < i_read; i++) {
            buffer[i] ^= psz_key[curoff ++];
...
static int Seek( stream_t *p_access, uint64_t i_pos )
{
    ...
    char buf[100];
    snprintf(buf, 100, "Range: bytes=%llu-\r\n", i_pos);
    
    if (Connect(p_access, buf) == 0) {
        p_sys->offset = i_pos;
        return VLC_SUCCESS;
    }
 ...
 * Connect:
 *****************************************************************************/
static int Connect( stream_t *p_access , const char * extheader)
{
    access_sys_t   *p_sys = p_access->p_sys;
    ...
    if (extheader != NULL) {
        vlc_memstream_puts( &stream, extheader);
    }

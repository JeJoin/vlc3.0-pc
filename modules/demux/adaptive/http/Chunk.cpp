...
block_t * AbstractChunk::doRead(size_t size, bool b_block, int nkey, const char * pkey)
{
    if(!source)
        return NULL;
    ...
    if(block)
    {
        ...
        if (nkey > 0 && pkey != NULL) {
            size_t off = bytesRead % nkey;
            for (size_t i = 0; i < block->i_buffer; i++) {
                block->p_buffer[i] ^= pkey[off ++];
...
block_t * AbstractChunk::readBlock(int nkey, const char * pkey)
{
    return doRead(0, true, nkey, pkey);
}

block_t * AbstractChunk::read(size_t size)
{
    return doRead(size, false, 0, NULL);
}

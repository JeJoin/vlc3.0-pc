...
block_t *hxxx_AnnexB_to_xVC( block_t *p_block, uint8_t i_nal_length_size )
{
    ...
    block_t * p_dupblock = block_Duplicate( p_block );
    if( unlikely(!p_dupblock) )
        goto error;
    
    block_t * p_oriblock = p_block;
    p_block = p_dupblock;
    
#if 1
    uint8_t * pdst = p_block->p_buffer;
    uint8_t * p_sourceend = p_oriblock->p_buffer + p_oriblock->i_buffer;
    
    for( unsigned i = 0; i < i_nalcount; i ++) {
        
        const uint8_t * p_start = p_list[i].p;
        const uint8_t * p_end = p_sourceend;
        if (i != i_nalcount - 1) {
            p_end = p_list[i + 1].p;
        }
        
        uint32_t i_payload = p_end - p_start - p_list[i].prefix;
        uint8_t type = p_start[p_list[i].prefix] & 0x1f;

        if (type == 0x07 || type == 0x08 || (type >= 1 && type <= 5)) {
            memcpy(pdst + i_nal_length_size, p_start + p_list[i].prefix, i_payload);
            hxxx_WritePrefix( i_nal_length_size, pdst, i_payload );
            pdst += i_nal_length_size + i_payload;
        }
    }
    p_block->i_buffer = pdst - p_block->p_buffer;
#else
...
error:
    free( p_list );
    block_Release( p_block );   //free duplicate block
    return NULL;
}
